...
requireRole(['admin']); // Use the requireRole function to restrict access to 'user' role only
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT id, title, description, due_date, due_time FROM reminders WHERE id = ? AND admin_id = ?");
    if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($due_date)) {
        ...
        if ($reminder_id_post && is_numeric($reminder_id_post)) {
            $stmt = $conn->prepare("UPDATE reminders SET title = ?, description = ?, due_date = ?, due_time = ?, updated_at = NOW() WHERE id = ? AND admin_id = ?");
            if ($stmt) {
...
$header_path = '../../templates/admin/header_admin.php'; // Default for user
$sidenav_path = '../../templates/admin/sidenav_admin.php'; // Default for user
...
<link rel="stylesheet" href="../../assets/css/admin_css/add_reminder.css">
