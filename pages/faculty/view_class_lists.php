<?php
// CHRONONAV_WEB_UNO/pages/faculty/view_class_lists.php
...
try {
    ...
    error_log("Database error fetching classes in view_class_lists.php: " . $e->getMessage());
    $errorMessage = "Could not retrieve class lists. Please try again later.";
    $classes = []; // Ensure $classes is empty on error
}

// Include header (adjust path relative to the current file)
            ...
            <tbody>
                <?php foreach ($classes as $class): ?>
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($class['day_of_week']) && !empty($class['start_time']) && !empty($class['end_time'])): ?>
...
<?php
// Include footer (adjust path relative to the current file)
include __DIR__ . '/../../templates/footer.php';
// The $conn connection should be closed in footer.php or when the script naturally ends.
?>