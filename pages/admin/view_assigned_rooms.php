...
try {
    if ($currentUserRole === 'faculty') {
        ...
        $stmt = $conn->prepare("
            SELECT DISTINCT r.room_id, r.room_name, r.capacity, c.class_name, c.class_code, c.day_of_week, c.start_time, c.end_time, c.semester
            FROM rooms r
        ...
        $stmt = $conn->prepare("
            SELECT r.room_id, r.room_name, r.capacity, c.class_name, c.class_code, u.name AS faculty_name, c.semester, c.day_of_week, c.start_time, c.end_time
            FROM rooms r
            LEFT JOIN classes c ON r.room_id = c.room_id
    ...
    error_log("Database error fetching assigned rooms: " . $e->getMessage());
